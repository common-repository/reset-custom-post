jQuery(document).ready(function($) {
    $('#mlzrcp_reset_cpt_button').on('click', function(e) {
        e.preventDefault();
        loaderShow();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.data.imagesIds !== undefined && response.data.imagesIds.length > 0) {
                            mlzrcp_reset_cptImages(postId, response.data.imagesIds, 0);
                    } 
    ...
    function mlzrcp_reset_cptImages(postId, imageIds, currentIndex) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#log').append('<p>'+response.data.log+'</p>');
                $('#log').scrollTop($('#log')[0].scrollHeight);
                mlzrcp_reset_cptImages(postId, imageIds, currentIndex + 1);
            },
            error: function(xhr, status, error) {
                console.error(error);
                loaderHide();
    ...
    $('#custom_post_type').on('change', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#mlzrcp_reset_cpt_button').attr('data-total', response.data.total);
                if (response.data.total == 0 && taxonomies == '') {
                    $('#mlzrcp_reset_cpt_button').attr('disabled', true);
                } else {
                    $('#mlzrcp_reset_cpt_button').attr('disabled', false);
                }
    ...
    function deleteElementsTaxonomies() {
        ...
        taxonomiesChecked.each(function(index, element) {
            ...
            var data = {
                action: 'mlzrcp_delete_elements_taxonomy',
